...
void main() {
  ...
  group('AnalyzerLocalDataSourceImpl', () {
    test('analyze should process text and save to database', () async {
      ...
      expect(result.totalWords, 4); // [hello, world, hello, again]
      expect(result.uniqueWords, 3); // [hello, world, again]
    ...
    test('analyze should reuse existing words and increment frequency', () async {
      // 1. First analysis
      await dataSource.analyze(title: 'T1', content: 'hello');
      
      final wordBefore = await (db.select(db.words)..where((w) => w.word.equals('hello'))).getSingle();
      expect(wordBefore.frequency, 1);

      // 2. Second analysis with same word
      await dataSource.analyze(title: 'T2', content: 'hello world');

      final wordAfter = await (db.select(db.words)..where((w) => w.word.equals('hello'))).getSingle();
