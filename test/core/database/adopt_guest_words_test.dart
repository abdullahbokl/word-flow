...
void main() {
  ...
  group('adoptGuestWords() optimization', () {
    test(
      'User with 0 existing words adopting 5 guest words enqueues exactly 5 items',
      () async {
        ...
        final userWords = await (db.select(
          db.words,
        )..where((t) => t.userId.equals('user-123'))).get();
        expect(userWords, hasLength(5));
      },
    );

    test(
      'User with existing words only enqueues newly migrated or merged words',
      () async {
    ...
    group('adoptGuestWords() merge strategy', () {
      test(
        'conflicting guest words update existing user word metrics',
        () async {
          const userId = 'user-123';
          ...
          final result = await (db.select(
            db.words,
          )..where((t) => t.id.equals('user-id-1'))).getSingle();
