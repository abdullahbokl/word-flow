...
void main() {
  ...
  group('Sync Queue Idempotency', () {
    test(
      'rapid upserts produce exactly 1 queue item and update timestamp',
      () async {
    ...
    test('retry count resets on re-queue of existing operation type', () async {
      ...
      expect(results, hasLength(1));
      expect(
        results.first.retryCount,
        0,
        reason: 'Retry count must reset to 0',
      );
      expect(
        results.first.lastError,
        isNull,
        reason: 'Last error must be cleared',
      );
    });

    test(
      'distinct operations for DIFFERENT words remain independent',
      () async {
