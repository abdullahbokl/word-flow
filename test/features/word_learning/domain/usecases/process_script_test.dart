...
void main() {
  ...
  test(
    'Sorts results: unknown words first, then by frequency descending',
    () async {
      // Arrange
  ...
  test(
    'Handles repository failure gracefully by using empty known set',
    () async {
      // Arrange
      ...
      const analysisResult = ScriptAnalysis(
        ...
        words: [
          ProcessedWord(wordText: 'sample', totalCount: 1, isKnown: false),
        ],
      );
  ...
  test('Sanitizes input before passing to analysis service', () async {
    ...
    when(
      ...
      (_) async =>
          const ScriptAnalysis(summary: ScriptSummary.empty(), words: []),
    );
