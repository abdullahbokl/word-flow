...
void main() {
  ...
  group('DeleteWord', () {
    ...
    test('should call repository.deleteWord with correct parameters', () async {
      when(
        () => mockRepository.deleteWord(any(), userId: any(named: 'userId')),
      ).thenAnswer((_) async => const Right(null));

      await useCase(const DeleteWordParams(id: testWordId, userId: testUserId));

      verify(
        () => mockRepository.deleteWord(testWordId, userId: testUserId),
      ).called(1);
    });

    test(
      'should call repository.deleteWord with null userId when not provided',
      () async {
        when(
          () => mockRepository.deleteWord(any(), userId: any(named: 'userId')),
        ).thenAnswer((_) async => const Right(null));
        ...
        verify(
          () => mockRepository.deleteWord(testWordId, userId: null),
        ).called(1);
      },
    );
    ...
    test('should return Right(null) on success', () async {
      ...
      final result = await useCase(
        const DeleteWordParams(id: testWordId, userId: testUserId),
      );
