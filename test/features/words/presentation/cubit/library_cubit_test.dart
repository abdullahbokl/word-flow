...
void main() {
  ...
  group('init - watches words stream', () {
    blocTest<LibraryCubit, LibraryState>(
      ...
      build: () {
        when(() => mockWatchWords(userId: any(named: 'userId')))
      ...
      verify: (cubit) {
        verify(() => mockWatchWords(userId: 'user-1')).called(1);
      },
    );
    ...
    blocTest<LibraryCubit, LibraryState>(
      ...
      verify: (cubit) {
        verify(() => mockWatchWords(userId: null)).called(1);
      },
    );
  });
  ...
  group('deleteWord - optimistic deletion', () {
    blocTest<LibraryCubit, LibraryState>(
      ...
      build: () {
        when(() => mockDeleteWord(any(), userId: any(named: 'userId')))
            .thenAnswer((_) async => const Right(null));
        return buildCubit();
      },
      ...
      verify: (cubit) {
        verify(() => mockDeleteWord(testWord.id, userId: 'user-1')).called(1);
      },
    );
    ...
    blocTest<LibraryCubit, LibraryState>(
      ...
      verify: (cubit) {
        verify(() => mockDeleteWord(testWord.id, userId: null)).called(1);
      },
    );
  });
